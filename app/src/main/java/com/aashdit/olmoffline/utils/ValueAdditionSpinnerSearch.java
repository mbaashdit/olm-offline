...
public class ValueAdditionSpinnerSearch extends AppCompatSpinner implements OnCancelListener {
	...
	private ValueAdditionSpinnerListener listener;
	private LimitExceedListener limitListener;

	private MyAdapter adapter;
	private List<ValueAdd> items;
	private boolean isSearchEnabled = true;

	public ValueAdditionSpinnerSearch(Context context) {
		super(context);
	}

	public ValueAdditionSpinnerSearch(Context arg0, AttributeSet arg1) {
		super(arg0, arg1);
	...
	public ValueAdditionSpinnerSearch(Context arg0, AttributeSet arg1, int arg2) {
	...
	public List<ValueAdd> getSelectedItems() {
		List<ValueAdd> selectedItems = new ArrayList<>();
		for (ValueAdd item : items) {
			if (item.isSelected) {
	...
	public List<Long> getSelectedIds() {
		...
		for (ValueAdd item : items) {
			if (item.isSelected) {
				selectedItemsIds.add(item.valueId);
			}
		}
		return selectedItemsIds;
	}

	public void refreshAdapterForUpdate() {
		if (adapter != null) {
...
//			setHintText("");
	...
	public void onCancel(DialogInterface dialog) {
		...
		ArrayList<ValueAdd> selectedData = new ArrayList<>();
		for (int i = 0; i < items.size(); i++) {
			ValueAdd currentData = items.get(i);
			if (currentData.isSelected) {
				selectedData.add(currentData);
				spinnerBuffer.append(currentData.valueEn);
	...
	public boolean performClick() {
		...
		if (isShowSelectAllButton && limit == -1) {
			builder.setNeutralButton(android.R.string.selectAll, (dialog, which) -> {
				for (int i = 0; i < adapter.arrayList.size(); i++) {
					adapter.arrayList.get(i).isSelected=true;
				}
				adapter.notifyDataSetChanged();
		...
		builder.setNegativeButton(clearText, new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				//Log.i(TAG, " ITEMS : " + items.size());
				for (int i = 0; i < adapter.arrayList.size(); i++) {
					adapter.arrayList.get(i).isSelected=false;
				}
				adapter.notifyDataSetChanged();
				dialog.cancel();
			}
	...
	public void setItems(List<ValueAdd> items, ValueAdditionSpinnerListener listener) {
		...
		for (int i = 0; i < items.size(); i++) {
			if (items.get(i).isSelected) {
				spinnerBuffer.append(items.get(i).valueEn);
				spinnerBuffer.append(", ");
			}
		}
	...
	public interface LimitExceedListener {
		void onLimitListener(ValueAdd data);
	}

	//Adapter Class
	public class MyAdapter extends BaseAdapter implements Filterable {

		final List<ValueAdd> mOriginalValues; // Original Values
		final LayoutInflater inflater;
		List<ValueAdd> arrayList;

		MyAdapter(Context context, List<ValueAdd> arrayList) {
...
//            //Log.i(TAG, "getView() enter");
			...
			final ValueAdd data = arrayList.get(position);

			holder.textView.setText(data.valueEn);
		...
		public Filter getFilter() {
			return new Filter() {
				...
				protected void publishResults(CharSequence constraint, FilterResults results) {
					...
					arrayList = (List<ValueAdd>) results.values; // has the filtered values
				...
				protected FilterResults performFiltering(CharSequence constraint) {
					...
					List<ValueAdd> FilteredArrList = new ArrayList<>();
					...
					if (constraint == null || constraint.length() == 0) {
						...
						for (int i = 0; i < mOriginalValues.size(); i++) {
							...
							String data = mOriginalValues.get(i).valueEn;
