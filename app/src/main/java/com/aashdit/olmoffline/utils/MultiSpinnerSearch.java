...
public class MultiSpinnerSearch extends AppCompatSpinner implements OnCancelListener {
	...
	private List<IrrigationSource> items;
	...
	public List<IrrigationSource> getSelectedItems() {
		List<IrrigationSource> selectedItems = new ArrayList<>();
		for (IrrigationSource item : items) {
			if (item.isSelected) {
	...
	public void onCancel(DialogInterface dialog) {
		...
		ArrayList<IrrigationSource> selectedData = new ArrayList<>();
		for (int i = 0; i < items.size(); i++) {
			IrrigationSource currentData = items.get(i);
	...
	public void setItems(List<IrrigationSource> items, MultiSpinnerListener listener) {
	...
	public interface LimitExceedListener {
		void onLimitListener(IrrigationSource data);
	}

	//Adapter Class
	public class MyAdapter extends BaseAdapter implements Filterable {

		final List<IrrigationSource> mOriginalValues; // Original Values
		final LayoutInflater inflater;
		List<IrrigationSource> arrayList;

		MyAdapter(Context context, List<IrrigationSource> arrayList) {
...
//            //Log.i(TAG, "getView() enter");
			...
			final IrrigationSource data = arrayList.get(position);
		...
		public Filter getFilter() {
			return new Filter() {
				...
				protected void publishResults(CharSequence constraint, FilterResults results) {
					...
					arrayList = (List<IrrigationSource>) results.values; // has the filtered values
				...
				protected FilterResults performFiltering(CharSequence constraint) {
					...
					List<IrrigationSource> FilteredArrList = new ArrayList<>();
